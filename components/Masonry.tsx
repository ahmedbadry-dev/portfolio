        ...
        new Promise<void>(resolve => {
          ...
          img.onload = () => {
            if (img.naturalWidth > 0 && img.naturalHeight > 0) {
              ratios[src] = img.naturalHeight / img.naturalWidth;
            }
            resolve();
          };
          img.onerror = () => resolve();
...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const { grid, totalHeight } = useMemo(() => {
    ...
    const computedHeight = Math.max(...colHeights, 0);
    const nextHeight = computedHeight > 0 ? computedHeight - gap : 0;

    return { grid: gridItems, totalHeight: nextHeight };
  ...
  return (
    <div
      ...
      style={{ minHeight: totalHeight || 360 }}
    >
      {grid.map(item => (
        <div
          key={item.id}
